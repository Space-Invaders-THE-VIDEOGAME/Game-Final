...
bool elimina_bala_objeto(struct Nave& N , struct Nave &E, struct Balas balas[]){

        if ( N.n_disp > 0 && N.n_disp < N.max_disp){
            for ( int cont = 1; cont <= N.n_disp; cont++){
                    if(colicion(E.x , E.y , E.ancho_p , E.alto_p , balas[cont].x , balas[cont].y ,N.ancho_b , N.alto_b ) && E.vida > 0){
                        eliminar(balas , N.n_disp,cont);
                        E.vida--;
                        return true;
                    }
